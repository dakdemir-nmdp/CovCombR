...
n_total <- 1000  # Total number of genotypes
n_markers_base <- 50000  # Number of base markers for population structure

# Create population structure: 3 subpopulations
subpop_sizes <- c(40, 35, 25)
subpop_labels <- rep(1:3, times = subpop_sizes)
genotype_ids <- paste0("G", sprintf("%03d", 1:n_total))

# Generate a realistic covariance structure for the base population
# Higher correlation within subpopulations
base_cor_within <- 0.6  # Within subpopulation correlation
base_cor_between <- 0.1  # Between subpopulation correlation

# Build correlation structure
true_cor_matrix <- matrix(base_cor_between, n_total, n_total)
for (i in 1:3) {
  idx <- which(subpop_labels == i)
  true_cor_matrix[idx, idx] <- base_cor_within
}
diag(true_cor_matrix) <- 1

# Add some random variation to make it more realistic
noise_matrix <- matrix(rnorm(n_total^2, 0, 0.05), n_total, n_total)
noise_matrix <- (noise_matrix + t(noise_matrix)) / 2
true_cor_matrix <- true_cor_matrix + noise_matrix

# Ensure positive definiteness
eig <- eigen(true_cor_matrix, symmetric = TRUE)
min_eig <- min(eig$values)
if (min_eig < 0.01) {
  true_cor_matrix <- eig$vectors %*% diag(pmax(eig$values, 0.01)) %*% t(eig$vectors)
}

# Standardize to correlation matrix
true_cor_matrix <- cov2cor(true_cor_matrix)
dimnames(true_cor_matrix) <- list(genotype_ids, genotype_ids)

cat("Base population structure created:\n")
cat(sprintf("  Total genotypes: %d\n", n_total))
cat(sprintf("  Subpopulation sizes: %s\n", paste(subpop_sizes, collapse = ", ")))
cat(sprintf("  Mean within-subpop correlation: %.3f\n",
            mean(true_cor_matrix[subpop_labels == 1, subpop_labels == 1][
              upper.tri(true_cor_matrix[subpop_labels == 1, subpop_labels == 1])])))
cat(sprintf("  Mean between-subpop correlation: %.3f\n",
            mean(true_cor_matrix[subpop_labels == 1, subpop_labels == 2])))
```

## Step 2: Helper Function for Van Raden GRM

```{r vanraden-function}
# Van Raden (2008) GRM calculation
calculate_vanraden_grm <- function(marker_matrix, genotype_names = NULL) {
  # marker_matrix: individuals x markers (0, 1, 2 coding)
  # Returns: GRM scaled to average diagonal = 1

  n <- nrow(marker_matrix)
  p <- ncol(marker_matrix)

  # Calculate allele frequencies
  allele_freq <- colMeans(marker_matrix, na.rm = TRUE) / 2

  # Center markers
  P <- matrix(2 * allele_freq, nrow = n, ncol = p, byrow = TRUE)
  Z <- marker_matrix - P

  # Calculate GRM
  ZZt <- tcrossprod(Z)

  # Scale by trace/n to get average diagonal = 1
  scaling_factor <- sum(diag(ZZt)) / n
  G <- ZZt / scaling_factor

  if (!is.null(genotype_names)) {
    dimnames(G) <- list(genotype_names, genotype_names)
  }
...
## Step 3: Generate Marker Data for Base Population

```{r base-markers}
# Generate base marker data that reflects the population structure
# This represents the "true" genetic data for all individuals and all available markers
# Sample GRMs will be calculated from subsets of these markers and individuals

# Generate correlated genotypes using the Cholesky decomposition
L <- chol(true_cor_matrix)
n_markers_total <- 50000  # Total markers available in the population

# For each marker, generate correlated values across individuals
# Then threshold to get 0, 1, 2 genotypes
marker_freqs_base <- runif(n_markers_total, 0.1, 0.9)
marker_matrix_base <- matrix(0, n_total, n_markers_total)

for (m in 1:n_markers_total) {
  # Generate correlated normal values
  z <- rnorm(n_total)
  corr_z <- crossprod(L, z)

  # Convert to genotypes using allele frequency
  p <- marker_freqs_base[m]
  # Thresholds for trinomial (0, 1, 2)
  thresh1 <- qnorm((1-p)^2)
  thresh2 <- qnorm((1-p)^2 + 2*p*(1-p))

  marker_matrix_base[, m] <- ifelse(corr_z < thresh1, 0,
                                     ifelse(corr_z < thresh2, 1, 2))
}

rownames(marker_matrix_base) <- genotype_ids
colnames(marker_matrix_base) <- paste0("M", 1:n_markers_total)

# Calculate the "true" GRM from ALL markers and ALL individuals
# This is the target we want to estimate
true_grm <- calculate_vanraden_grm(marker_matrix_base, genotype_ids)

cat("\nBase population marker data generated:\n")
cat(sprintf("  Total markers available: %d\n", n_markers_total))
cat(sprintf("  Total genotypes: %d\n", n_total))
cat("\nTrue population GRM calculated (from all markers):\n")
cat(sprintf("  Mean diagonal: %.3f\n", mean(diag(true_grm))))
cat(sprintf("  Mean off-diagonal: %.3f\n", mean(true_grm[upper.tri(true_grm)])))
cat(sprintf("  SD off-diagonal: %.3f\n", sd(true_grm[upper.tri(true_grm)])))
```

## Step 4: Generate 10 Study-Specific GRMs

Now we create 10 different studies, each with:
- A different subset of genotypes following population structure but with intentional overlaps
- A different subset of markers sampled from the base population
- GRMs calculated only from observed markers and individuals

```{r study-grms}
# Study design parameters
n_studies <- 10
n_markers_per_study <- 5000  # Markers per study (sampled from the 10,000 total)
min_genotypes_per_study <- 200
max_genotypes_per_study <- 400

# Storage for study-specific data
study_genotypes <- list()
study_markers <- list()
study_grms <- list()
study_marker_counts <- integer(n_studies)

set.seed(2025)

# Design study sampling patterns that follow population structure
# but ensure intentional overlaps
for (k in 1:n_studies) {

  # Select subset of genotypes following population structure with intentional overlaps
  if (k <= 3) {
    # First 3 studies: Focus on each subpopulation separately (with some spillover)
    target_subpop <- k
    n_study <- sample(min_genotypes_per_study:max_genotypes_per_study, 1)

    # 70% from target subpopulation, 30% from others
    target_idx <- which(subpop_labels == target_subpop)
    other_idx <- which(subpop_labels != target_subpop)

    n_from_target <- round(n_study * 0.7)
    n_from_other <- n_study - n_from_target

    study_idx <- sort(c(
      sample(target_idx, min(n_from_target, length(target_idx))),
      sample(other_idx, min(n_from_other, length(other_idx)))
    ))

  } else if (k <= 6) {
    # Studies 4-6: Mix of two subpopulations (intentional overlap)
    subpop_pair <- list(c(1,2), c(2,3), c(1,3))[[k-3]]
    n_study <- sample(min_genotypes_per_study:max_genotypes_per_study, 1)

    target_idx <- which(subpop_labels %in% subpop_pair)
    study_idx <- sort(sample(target_idx, min(n_study, length(target_idx))))

  } else {
    # Studies 7+: Random sampling across all subpopulations
    # with higher probability for individuals already in other studies (intentional overlap)
    # Also ensure all genotypes are eventually covered
    n_study <- sample(min_genotypes_per_study:max_genotypes_per_study, 1)

    prev_genotypes <- unique(unlist(study_genotypes))
    uncovered_genotypes <- setdiff(1:n_total, prev_genotypes)

    # If there are uncovered genotypes and we're past study 10, ensure we include some
    if (k > 10 && length(uncovered_genotypes) > 0) {
      # Force inclusion of some uncovered genotypes
      n_uncovered <- min(round(n_study * 0.3), length(uncovered_genotypes))
      forced_idx <- sample(uncovered_genotypes, n_uncovered)

      # Sample remaining from all genotypes with higher prob for previous
      prob_include <- rep(0.3, n_total)
      prob_include[prev_genotypes] <- 0.6
      prob_include[forced_idx] <- 0  # Already forced in

      remaining_idx <- sample(1:n_total, n_study - n_uncovered, prob = prob_include)
      study_idx <- sort(c(forced_idx, remaining_idx))
    } else {
      prob_include <- rep(0.3, n_total)
      prob_include[prev_genotypes] <- 0.6  # Higher prob for already-sampled individuals

      study_idx <- sample(1:n_total, n_study, prob = prob_include)
      study_idx <- sort(study_idx)
    }
  }

  study_genotypes[[k]] <- study_idx
  study_ids <- genotype_ids[study_idx]
  n_study <- length(study_idx)

  # Sample markers from the base population
  # Different studies observe different marker panels
  # Allow some overlap between studies (30-50% shared markers)
  if (k == 1) {
    # First study: random marker selection
    marker_idx <- sort(sample(1:n_markers_total, n_markers_per_study))
  } else {
    # Subsequent studies: partial overlap with previous studies
    prev_markers <- unique(unlist(study_markers[1:(k-1)]))

    # 40% overlap with previously used markers, 60% new markers
    n_overlap <- round(n_markers_per_study * 0.4)
    n_new <- n_markers_per_study - n_overlap

    if (length(prev_markers) >= n_overlap) {
      overlap_markers <- sample(prev_markers, n_overlap)
    } else {
      overlap_markers <- prev_markers
      n_new <- n_markers_per_study - length(prev_markers)
    }

    available_new <- setdiff(1:n_markers_total, overlap_markers)
    new_markers <- sample(available_new, min(n_new, length(available_new)))

    marker_idx <- sort(c(overlap_markers, new_markers))
  }

  study_markers[[k]] <- marker_idx

  # Extract the marker data for this study's individuals and markers
  study_marker_matrix <- marker_matrix_base[study_idx, marker_idx, drop = FALSE]

  # Calculate Van Raden GRM for this study using only observed data
  study_grm <- calculate_vanraden_grm(study_marker_matrix, study_ids)
  study_grms[[k]] <- study_grm
  study_marker_counts[k] <- length(marker_idx)

  # Report subpopulation composition
  subpop_counts <- table(subpop_labels[study_idx])
  subpop_str <- paste(sprintf("Pop%d:%d", as.numeric(names(subpop_counts)),
                             subpop_counts), collapse = ", ")

  cat(sprintf("Study %2d: %2d genotypes (%s), %4d markers\n",
              k, n_study, subpop_str, study_marker_counts[k]))
}

names(study_grms) <- paste0("Study", 1:n_studies)
names(study_genotypes) <- paste0("Study", 1:n_studies)
names(study_markers) <- paste0("Study", 1:n_studies)
```
...
for (i in 1:n_studies) {
  for (j in 1:n_studies) {
    genotype_overlap_matrix[i, j] <- length(intersect(study_genotypes[[i]],
                                                       study_genotypes[[j]]))
  }
}
...
for (i in 1:n_studies) {
  for (j in 1:n_studies) {
    marker_overlap_matrix[i, j] <- length(intersect(study_markers[[i]],
                                                     study_markers[[j]]))
  }
}
...
cat(sprintf("  Genotypes: %d / %d covered (%.1f%%)\n",
...
cat(sprintf("  Mean studies per genotype: %.1f (range: %d-%d)\n",
            mean(genotype_frequency),
            min(genotype_frequency), max(genotype_frequency)))

all_markers_used <- unique(unlist(study_markers))
cat(sprintf("  Markers: %d / %d used (%.1f%%)\n",
            length(all_markers_used), n_markers_total,
            100 * length(all_markers_used) / n_markers_total))

marker_frequency <- table(unlist(study_markers))
cat(sprintf("  Mean studies per marker: %.1f (range: %d-%d)\n",
            mean(marker_frequency),
            min(marker_frequency), max(marker_frequency)))
```

# Visualization of Sample GRMs

Now we visualize all 10 sample GRMs along with the true population GRM.

```{r heatmap-function}
# Helper function to create a GRM heatmap
plot_grm_heatmap <- function(grm_matrix, title = "GRM",
                             text_size = 6, show_values = FALSE) {
  n <- nrow(grm_matrix)
  ...
  grm_long$value <- as.vector(grm_matrix)
  grm_long$Row_id <- rownames(grm_matrix)[grm_long$Row]
  grm_long$Col_id <- colnames(grm_matrix)[grm_long$Col]

  # Create plot
  p <- ggplot(grm_long, aes(x = Col, y = n - Row + 1, fill = value)) +
    geom_tile() +
...
```{r visualize-all-grms, fig.width=9, fig.height=18, out.width="100%", fig.alt="Heatmaps of the true population GRM and each study-specific GRM before alignment."}
# Create heatmaps for all studies
study_plots <- lapply(1:n_studies, function(k) {
  plot_grm_heatmap(
    study_grms[[k]],
    title = sprintf("Study %d\n(n=%d)", k, length(study_genotypes[[k]])),
    text_size = 8
  )
})

# Create heatmap for true GRM (full population)
true_grm_plot <- plot_grm_heatmap(
  true_grm,
  title = sprintf("True Population GRM\n(n=%d)", n_total),
  text_size = 8
)

# Arrange all plots in a grid (1 true + n_studies)
all_plots <- c(list(true_grm_plot), study_plots)
n_plots <- length(all_plots)
n_cols <- 3
n_rows <- ceiling(n_plots / n_cols)
grid.arrange(grobs = all_plots, ncol = n_cols, nrow = n_rows)
```

```{r visualize-study-subset, fig.width=8, fig.height=7, out.width="100%", fig.alt="Heatmaps for the first four study GRMs highlighting sample size and marker coverage."}
# Show first 4 studies in more detail
study_plots_detail <- lapply(1:4, function(k) {
  plot_grm_heatmap(
    study_grms[[k]],
    title = sprintf("Study %d (n=%d, m=%d markers)",
                   k, length(study_genotypes[[k]]), study_marker_counts[k]),
    text_size = 10
  )
})

grid.arrange(grobs = study_plots_detail, ncol = 2, nrow = 2)
```
...
if (length(well_sampled_genotypes) >= 10) {
  ...
  selected_genotypes <- sample(well_sampled_genotypes,
  ...
  cat(sprintf("  Number of genotype pairs compared: %d\n",
              nrow(comparison_df)))
  cat(sprintf("  RMSE: %.4f\n", sqrt(mean(comparison_df$Error^2))))
  cat(sprintf("  MAE: %.4f\n", mean(abs(comparison_df$Error))))
  cat(sprintf("  Correlation: %.4f\n",
              cor(comparison_df$True_Relationship,
...
for (k in 1:n_studies) {
  cat(sprintf("  Study %2d: %d genotypes, nu = %d\n",
...
plot(fit$history$log_likelihood, type = "o", xlab = "Iteration", ylab = "Log-Likelihood",
...
cat(sprintf("  Genotypes in combined GRM: %d / %d\n",
...
cat(sprintf("  N pairs: %d (%.1f%% of total)\n",
            sum(valid_idx),
...
for (pop1 in 1:3) {
  for (pop2 in pop1:3) {
    ...
    if (sum(block_valid) > 0) {
      ...
      cat(sprintf("  Pop%d-Pop%d: N=%d, RMSE=%.4f, Cor=%.4f\n",
