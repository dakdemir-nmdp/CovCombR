...
**Scope note**: Every result below — chordality, path-distance error scaling, the SE-inflation theorems — applies to the **unconstrained free-$\Sigma$ model** (`n_factors = NULL`). Since CovCombR 1.6, `fit_covcomb()` defaults to `n_factors = "auto"`, which can select a **factor model** ($\Sigma = \Lambda\Lambda^\top + \Psi$) instead. The factor model extends identifiability beyond chordal coverage: an entry $\Sigma_{ij}$ can be identified through shared factor loadings even when the observation graph is non-chordal, or when $(i,j)$ lies outside any chordal completion. The code examples in this document explicitly pass `n_factors = NULL` so that the numerical results match the free-$\Sigma$ theory developed here; see `vignette("statistical-methods")` for the factor-model identifiability conditions.
...
# n_factors = NULL: this example illustrates free-Sigma identifiability theory,
# so the free model is fit explicitly rather than relying on auto-selection.
...
boot_vals <- replicate(B, {
  ...
  fit_b <- fit_covcomb(S_list_b, nu, n_factors = NULL, se_method = "none")
...
data.frame(
  Method = c("Bootstrap", "Naive plugin"),
  SE     = round(c(SE_boot, SE_plugin), 4)
)

SE_boot / SE_plugin  # ratio: bootstrap SE relative to naive plugin SE
```
