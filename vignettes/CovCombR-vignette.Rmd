...
matrix_to_long <- function(mat, label) {
  mat <- as.matrix(mat)
  nr <- nrow(mat)
  nc <- ncol(mat)
  row_nm <- rownames(mat)
  col_nm <- colnames(mat)
  if (is.null(row_nm)) row_nm <- paste0("Row", seq_len(nr))
  if (is.null(col_nm)) col_nm <- paste0("Col", seq_len(nc))
  grid <- expand.grid(
    row_idx = seq_len(nr),
    col_idx = seq_len(nc),
  ...
  grid$row <- row_nm[grid$row_idx]
  grid$col <- col_nm[grid$col_idx]
  grid$value <- mat[cbind(grid$row_idx, grid$col_idx)]
  grid$matrix <- label
  grid[, c("row", "col", "value", "matrix"), drop = FALSE]
}

plot_data <- bind_rows(
  matrix_to_long(true_cov, "True"),
  matrix_to_long(naive_cov, "Naive"),
  matrix_to_long(em_cov, "CovCombR")
)
...
plot_data <- plot_data |> mutate(label = ifelse(is.na(value), "", sprintf("%.2f", value)))

value_limits <- range(plot_data$value, na.rm = TRUE)

ggplot(plot_data, aes(x = col, y = row, fill = value)) +
  geom_tile(color = "white", linewidth = 0.2) +
  geom_text(aes(label = label), color = "black", size = 2.5, na.rm = TRUE) +
  scale_fill_gradient2(limits = value_limits, midpoint = 0, na.value = "#2F2F2F",
...
corr_plot_data <- bind_rows(
  matrix_to_long(true_corr, "True"),
  matrix_to_long(naive_corr, "Naive"),
  matrix_to_long(em_corr, "CovCombR")
)
...
corr_plot_data <- corr_plot_data |> mutate(label = ifelse(is.na(value), "", sprintf("%.2f", value)))

corr_value_limits <- range(corr_plot_data$value, na.rm = TRUE)

ggplot(corr_plot_data, aes(x = col, y = row, fill = value)) +
  geom_tile(color = "white", linewidth = 0.2) +
  geom_text(aes(label = label), color = "black", size = 2.5, na.rm = TRUE) +
  scale_fill_gradient2(limits = corr_value_limits, midpoint = 0, na.value = "#2F2F2F",
                       low = "#2166AC", high = "#B2182B", mid = "#F7F7F7",
                       name = "Correlation") +
  facet_wrap(~matrix, nrow = 1) +
  coord_equal() +
