...
for (i in 1:nrow(design)) {
  ...
  cat(sprintf("\n=== Scenario %d/%d ===\n", i, nrow(design)))
  cat(sprintf("  rho_13 = %.2f, nu = %d, SE method = %s\n",
  ...
  cat(sprintf("  Progress: %.0f%% | Elapsed: %.1f min | Est. total: %.1f min\n",
...
for (i in 1:nrow(demo_design)) {
  ...
  cat(sprintf("Scenario %d/%d: rho_13=%.1f, nu=%d\n",
...
```{r power_heatmap, fig.width = 8, fig.height = 6}
ggplot(results, aes(x = factor(rho_13_true), y = factor(nu), fill = power)) +
  geom_tile(color = "white", size = 1) +
  geom_text(aes(label = sprintf("%.2f", power)), color = "white", size = 4.5, fontface = "bold") +
  scale_fill_gradient2(
    low = "#FEE5D9", mid = "#6BAED6", high = "#08306B",
    midpoint = 0.5, limits = c(0, 1),
    name = "Power"
  ) +
  geom_hline(yintercept = 1.5, linetype = "dashed", color = "gray40", alpha = 0.5) +
  geom_hline(yintercept = 2.5, linetype = "dashed", color = "gray40", alpha = 0.5) +
  geom_hline(yintercept = 3.5, linetype = "dashed", color = "gray40", alpha = 0.5) +
  labs(
    title = "Power Landscape: Sample Size vs. Effect Size (Bootstrap)",
    subtitle = "Testing unobserved correlation between variables never jointly measured",
    x = "True Correlation (ρ₁₃)",
    y = "Sample Size (ν = ν₁ = ν₂)",
    caption = "Cells show empirical power from M=20 Monte Carlo replicates (B=30 bootstrap per fit)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
