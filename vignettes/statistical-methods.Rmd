...
where the Wishart density is:

$$p(\mathbf{W}_k \mid \boldsymbol{\Sigma}, \alpha_k, \gamma, \nu_k) \propto |\mathbf{W}_k|^{(\nu_k - p - 1)/2} \exp\left(-\frac{1}{2\alpha_k\gamma} \text{tr}(\boldsymbol{\Sigma}^{-1} \mathbf{W}_k)\right)$$
...
- `log_likelihood`: Observed-data log-likelihood $\mathcal{L}(\boldsymbol{\theta}^{(t)})$
...
## Comparison: Plugin vs. Bootstrap

| Criterion | Plugin | Bootstrap |
|-----------|--------|-----------|
| **Speed** | Fast (~1 sec) | Slow (~2 min for B=200) |
| **Accuracy** | Approximate | More accurate |
| **Sample size** | Large $\nu_k$ | Any $\nu_k$ |
| **Identifiability** | Explicit (returns NA) | Implicit (high SE) |
| **Reproducibility** | Deterministic | Stochastic (use seed) |
| **Recommendation** | Default for exploration | Use for final inference |
